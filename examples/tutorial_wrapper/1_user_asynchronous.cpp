...
// This file should only be used for the user to take specific examp	les.
...
public:
    void display(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr)
    {
        ...
        {
			cv::namedWindow("User worker GUI2", cv::WINDOW_AUTOSIZE);
            cv::imshow("User worker GUI", datumsPtr->at(0).cvOutputData);
			cv::Mat test;
			test = cv::imread("C:/openpose/examples/media/test.jpg", 1);
			cv::imshow("User worker GUI2", test);
