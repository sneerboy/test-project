...
namespace op
{
    ...
    void Gui::update(const cv::Mat& cvOutputData)
    {
        try
        {
			...
			cv::Mat tmp = cv::imread("C:/openpose/examples/media/test.jpg");
			cv::Mat tmp2 = cv::imread("C:/openpose/examples/media/no_img.jpg");
            const bool returnedIsValidFrame = ((spIsRunning == nullptr || *spIsRunning) && !cvOutputData.empty());
			//std::cout << joon1 << std::endl;
			

			///////////file load test// 
			/*cv::FileStorage fs2("C:/openpose/examples/media/aa_pose.yml", cv::FileStorage::READ);
			cv::Mat load_mat;
			fs2["pose_0"] >> load_mat;
			std::cout << load_mat.at<float>(1) << std::endl;*/

			///////////////

			if (60 < joon1 && joon1 < 90) {
				cv::imshow("tt", tmp);
			}
			else {
				cv::imshow("tt", tmp2);
